...
class PackReplica:
    def __init__(self,L,N,outFileNum,pressure,randomSeed,initialDensity, R):
        ...
        self.r=(initialDensity*L**3/(4.*self.N*np.pi/3.))**(1./3.)
        
        #self.V=self.N*np.pi/initialDensity
        self.V=(4./3.)*np.pi/initialDensity
    ...
    def attemptExpansion(self):
        ...
        if expansionFlag==True:
            ...
            newDensity=self.N*(4./3.)*3.14159/self.V #assumes particle radius 1
            ...
            self.r=(initialDensity*L**3/(4.*np.pi*self.N/3.))**(1./3.)
    ...
    def getDensity(self):
        ...
        self.density=self.N*(4./3.)*3.14159*self.r**3/self.L**3
...
while simNum<numSims:
    ...
    pr = PackReplica(L=L,N=N,outFileNum=simNum,pressure=pressure,randomSeed=randomSeed,initialDensity=initialDensity, R=R)
    pr.initialize()
