...
class PackReplica:
    ...
    def checkIntersections(self,i):
        ...
        lati=self.coords[i][0]
        loni=self.coords[i][1]
        xi=self.R*np.cos(lati)*np.cos(loni)
        yi=self.R*np.cos(lati)*np.sin(loni)
        zi=self.R*np.sin(lati)

        latn=neighbors[:,0]
        lonn=neighbors[:,1]
        xn=self.R*np.cos(latn)*np.cos(lonn)
        yn=self.R*np.cos(latn)*np.sin(lonn)
        zn=self.R*np.sin(latn)
