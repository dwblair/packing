import numpy as np
import random
import time


class PackReplica:
    def __init__(self,L,N):
        self.L=L
        self.N=N

        self.r=20.
        self.r=20. #particle radius

        self.t=0
        self.maxt=1000000

        self.coords=np.zeros((N,2))+L/2.
        self.colors=np.zeros(N)
        self.cellNums=np.zeros(N)
        self.maxNumberParticlesPerCell=10  # assumes no more than 10 particles per cell! may need to modify if polydisperse ...
        self.radii=np.zeros(N)+r
        self.stepSize=r*.1
        self.rScale=1.
        self.cellSize=10

        # params for moving algorithm
        self.mu = 0.
        self.translationStep=1.


    
timeGapForPrintout=1000

L=2**9 #system size, compatible with size of display in processing -- and must be a power of 2 for assignCell to work
N=100 #number of particles
r=20. #particle radius

t=0
maxt=1000000


coords=np.zeros((N,2))+L/2.
colors=np.zeros(N)
cellNums=np.zeros(N)
maxNumberParticlesPerCell=10  # assumes no more than 10 particles per cell! may need to modify if polydisperse ...
radii=np.zeros(N)+r
stepSize=r*.1
rScale=1.
cellSize=10


# params for moving algorithm
mu = 0.
translationStep=1.

def getMSD():
    coords
    
def checkIntersections(i):
    neighborIndices=np.concatenate((np.arange(0,i,1),np.arange(i+1,N,1)),axis=0)
    #i.e. all the coords except i, for N**2
    #neighborIndices=np.arange(0,N)
    neighbors=coords[neighborIndices]
    dx=neighbors[:,0]-coords[i][0]
    dy=neighbors[:,1]-coords[i][1]
    r2=dx*dx+dy*dy
    intersectionIndices=r2<((2*r*rScale)**2) #i.e. less than a diameter
    numIntersections=len(r2[intersectionIndices])

    x=coords[i][0]
    y=coords[i][1]

    # in a box
    if x<(0+radii[i]) or x>(L-radii[i]):
        numIntersections=numIntersections+1
    if y<(0+radii[i]) or y>(L-radii[i]):
        numIntersections=numIntersections+1
    return numIntersections
    
def getCellSize():
    maxDiam=2*radii.max()*rScale
    power=np.ceil(np.log2(maxDiam))
    cellSize=2**power
    return cellSize

def assignCells(): #assign a cell number to each particle
    #maxDiam=2*radii.max()*rScale
    #power=np.ceil(np.log2(maxDiam))
    #cellSize=2**power
    cellSize=getCellSize()
    cellRows=L/cellSize
    x=coords[:,0]
    y=coords[:,1]
    xCells=np.floor(x/cellSize)
    yCells=np.floor(y/cellSize)
    cellNums=yCells*cellRows+xCells
    #print cellNums
    #print "cellSize=",cellSize
    #cellContents=np.zeros((numCells,maxNumberParticlesPerCell))-1.
    return cellNums

def assignCellContents(): #put all particles with same cell number in cells
    cellSize=getCellSize() # double the work simply because I don't know how to do global variables!!
    cellRows=L/cellSize
    numCells=cellRows**2
    
    cellNums=assignCells()
    cellContents=np.zeros((numCells,maxNumberParticlesPerCell))-1. #-1 indicates not a real particle
    #cell
    # how to redo this for loop?
    # for c in range (0,numCells):
    #    thisCell cellNums==c
    #print cellContents
    
    #print cellNums
    
    


def printOut():

    #test of assignCells
    cellNums=assignCells()
    #assignCells()
    cellSize=getCellSize()
    #print cellNums
    assignCellContents()
    f=open('coords.txt','a')

    for i in range(0,N):
        thisline=str(coords[i][0])+","+str(coords[i][1])+","+str(radii[i]*rScale)+","+str(cellNums[i])+"\n"
        f.write(thisline)
            
    thisline="@ "+str(t)+" "+str(cellSize)+"\n"
    f.write(thisline)
    f.close()


# put coordinates in a grid
factor=1.5
x=factor*2*r
y=factor*2*r

for i in range(0,N):
    coords[i][0]=x;
    coords[i][1]=y;
    x=x+factor*2*r
    if x>(L-factor*r):
        x=factor*2*r
        y=y+factor*2*r
    radii[i]=r

timer_t=time.time()



for t in range(0, maxt):

    #pick a random circle
    i=random.randint(0,N-1)
    #generate random displacements in x and y
    oldCoords=np.copy(coords[i])
    # generate random step
    dr=np.random.normal(mu,translationStep,2) #array of size 2, for each coordinate
    # move the particle
    coords[i]=coords[i]+dr

    if (checkIntersections(i))>0:
        coords[i]=oldCoords
    
    if t%timeGapForPrintout==0:
        printOut()
        print "%.7g" % float((time.time()-timer_t)/timeGapForPrintout)
        timer_t=time.time()
