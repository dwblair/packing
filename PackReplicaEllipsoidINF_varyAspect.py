...
# run as "python PackReplica.py numSweeps tmax numSims N aspectRatio seedNum"
numSweeps=int(sys.argv[1])
tmax=int(sys.argv[2])
numSims=int(sys.argv[3])
N=int(sys.argv[4])
#outLabel=str(sys.argv[5])
aspectRatio=float(sys.argv[5])
seedNum=int(sys.argv[6])

#utLabel="seed"+str(seedNum)
...
while simNum<numSims:
    ...
    outLabel="aspect_"+str(aspectRatio)+"seedINF_"+str("%03d" % randomSeed)
    ...
    pr.printOut('highestDensity', "seedINFhighest_"+str("%03d" % randomSeed))
